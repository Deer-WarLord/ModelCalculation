...
class RearmingSimulation:
    ...
    def _build_capital_eq(self, j, i):
        self.EQ["K_old_{i}_{N}".format(N=j, i=i)] = (-self.EQ["mu_{i}".format(i=i)] *
                                                     ...
                                                     self.EQ["Y0_old_{i}".format(i=i)] -
                                                     self.EQ["Y1_old_{i}".format(i=i)] -
                                                     self.EQ["w_{i}_{pN}".format(pN=j - self.dt, i=i)] *
                                                     self.EQ["L_old_{i}_{pN}".format(pN=j - self.dt, i=i)] +
                                                     self.EQ["p_old_{i}_{pN}".format(pN=j - self.dt, i=i)] *
                                                     self.EQ["delta_old_{i}".format(i=i)] *
                                                     self.EQ["X_old_{i}_{pN}".format(pN=j - self.dt,
                                                                                   i=i)]) * self.dt + \
                                                    self.EQ["K_old_{i}_{pN}".format(pN=j - self.dt, i=i)]

    def _build_capital_eq_new(self, j, i):
        self.EQ["K_new_{i}_{N}".format(N=j, i=i)] = (-self.EQ["mu_{i}".format(i=i)] *
                                                     self.EQ["K_new_{i}_{pN}".format(pN=j - self.dt, i=i)] -
                                                     self.EQ["Y0_new_{i}".format(i=i)] -
                                                     self.EQ["Y1_new_{i}".format(i=i)] -
                                                     self.EQ["w_{i}_{pN}".format(pN=j - self.dt, i=i)] *
                                                     self.EQ["L_new_{i}_{pN}".format(pN=j - self.dt, i=i)] +
                                                     self.EQ["p_new_{i}_{pN}".format(pN=j - self.dt, i=i)] *
                                                     self.EQ["delta_new_{i}".format(i=i)] *
                                                     self.EQ["X_new_{i}_{pN}".format(pN=j - self.dt, i=i)] +
                                                     self.EQ["I_new_{i}_{N}".format(N=j, i=i)]) * self.dt + \
                                                    self.EQ["K_new_{i}_{pN}".format(pN=j - self.dt, i=i)]

    def _build_production_eq(self, j, prefix="_old"):
        for i in range(0, 3):
            self.EQ["X{}_{i}_{N}".format(prefix, N=j, i=i)] = self.EQ["A{}_{i}".format(prefix, i=i)] * \
                                                        self.EQ["L{}_{i}_{N}".format(prefix, N=j, i=i)] ** \
                                                              ((self.EQ["a_{i}".format(i=i)] +
                                                                self.EQ["b_{i}".format(i=i)] *
                                                                self.EQ["q_{N}".format(N=j)]) *
                                                               self.EQ["q_{N}".format(i=i, N=j)]) * \
                                                        self.EQ["K{}_{i}_{N}".format(prefix, N=j, i=i)] ** \
                                                              ((self.EQ["c_{i}".format(i=i)] +
                                                                self.EQ["d_{i}".format(i=i)] *
                                                                self.EQ["q_{N}".format(i=i, N=j)]) *
                                                               self.EQ["q_{N}".format(i=i, N=j)])
    ...
    def _price_eq(self, i, j, prefix="_old"):
        self.EQ["p{}_{i}_{N}".format(prefix, N=j, i=i)] = (self.EQ["Y0{}_{i}".format(prefix, i=i)] +
                                                           self.EQ["Y1{}_{i}".format(prefix, i=i)] +
                                                           self.EQ["w_{i}_{N}".format(N=j, i=i)] *
                                                           self.EQ["L{}_{i}_{N}".format(prefix, N=j, i=i)]) / \
                                                          (self.EQ["delta{}_{i}".format(prefix, i=i)] *
                                                           self.EQ["X{}_{i}_{N}".format(prefix, N=j, i=i)])

    def _build_labor_eq(self, i, j, prefix="_old"):
        self.EQ["L{}_{i}_{N}".format(prefix, N=j, i=i)] = self.EQ["L_{N}".format(N=j)] * \
                                                          self.EQ["theta{}_{i}_{N}".format(prefix, N=j, i=i)]

    def _build_supply_cond(self, j, prefix="_old"):
        self.COND["supply{}_{N}".format(prefix, N=j)] = (self.EQ["L{}_0_{N}".format(prefix, N=j)] *
                                                         self.EQ["w_0_{N}".format(N=j)] +
                                                         self.EQ["L{}_1_{N}".format(prefix, N=j)] *
                                                         self.EQ["w_1_{N}".format(N=j)] +
                                                         self.EQ["L{}_2_{N}".format(prefix, N=j)] *
                                                         self.EQ["w_2_{N}".format(N=j)]) - \
                                                        (self.EQ["p{}_2_{N}".format(prefix, N=j)] *
                                                         self.EQ["delta{}_2".format(prefix)] *
                                                         self.EQ["X{}_2_{N}".format(prefix, N=j)]) # >=0

    def _build_theta_cond(self, j):
        self.COND["theta_balance_{N}".format(N=j)] = (self.EQ["theta_new_0_{N}".format(N=j)] +
                                                      self.EQ["theta_new_1_{N}".format(N=j)] +
                                                      self.EQ["theta_new_2_{N}".format(N=j)]) - \
                                                     (self.EQ["theta_old_0_{N}".format(N=j)] +
                                                      self.EQ["theta_old_1_{N}".format(N=j)] +
                                                      self.EQ["theta_old_2_{N}".format(N=j)]) # == 0

    def _build_invest_cond(self, j):
        self.COND["invest_{N}".format(N=j)] = self.EQ["I_max"] - (self.EQ["I_new_0_{N}".format(N=j)] +
                                                                  self.EQ["I_new_1_{N}".format(N=j)] +
                                                                  self.EQ["I_new_2_{N}".format(N=j)]) # >=0

    def _build_salary_cond(self, j):
        for i in range(0, 3):
    ...
    def init_equation_system(self):
        ...
        self.EQ = {"L_{N0}".format(N0=0.0): self.json_initial_data["L0"],
                   "alpha": float(self.json_initial_data["alpha"]),
                   "beta": float(self.json_initial_data["beta"]),
                   "gamma": float(self.json_initial_data["gamma"]),
        ...
        for i in range(0, 3):
            ...
            self.EQ["Y_old_0_{i}".format(i=i)] = float(self.json_initial_data["Y_old_0"][i])
            self.EQ["Y_old_1_{i}".format(i=i)] = float(self.json_initial_data["Y_old_1"][i])

            self.EQ["delta_old_{i}".format(i=i)] = float(self.json_initial_data["delta_old"][i])

            self.EQ["W_{i}".format(i=i)] = float(self.json_initial_data["W"][i])

            self.EQ["a_{i}".format(i=i)] = float(self.json_initial_data["a"][i])
            self.EQ["b_{i}".format(i=i)] = float(self.json_initial_data["b"][i])
            self.EQ["c_{i}".format(i=i)] = float(self.json_initial_data["c"][i])
            self.EQ["d_{i}".format(i=i)] = float(self.json_initial_data["d"][i])

            self.EQ["A_new_{i}".format(i=i)] = float(self.json_initial_data["A_new"][i])

            self.EQ["Y_new_0_{i}".format(i=i)] = float(self.json_initial_data["Y_old_0"][i])
            self.EQ["Y_new_1_{i}".format(i=i)] = float(self.json_initial_data["Y_old_1"][i])

            self.EQ["delta_new_{i}".format(i=i)] = float(self.json_initial_data["delta_old"][i])
